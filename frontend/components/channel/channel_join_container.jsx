...
class JoinChannelForm extends React.Component {
  ...
  joinChannel(publicChannel) {
    return (e) => {
      const joinedChannelIds = this.props.getJoinedChannels.map(
        (channel) => channel.id
      );
  ...
  filterChannelList(e) {
    ...
    this.setState({
      publicChannels: this.props.getAllPublicChannels.filter((channel) =>
  ...
  render() {
    const publicChannels = this.state.publicChannels.map((publicChannel) => {
      ...
      const channelAuthor = channelUsers.filter(
        (user) => user.id === publicChannel.author_id
      )[0];
      return (
        <div onClick={this.joinChannel(publicChannel).bind(this)}>
          ...
          <li key={`public_channel_${publicChannel.id}`}>
            {publicChannel.name}
          </li>
        </div>
      );
    ...
    return (
      <div className="join-channel">
        ...
        <input
          className="public-channel-search"
          type="text"
          onChange={this.filterChannelList}
          placeholder="Search Channels"
        />
        <ul className="join-channel-pub">{publicChannels}</ul>
      </div>
    );
  }
}
